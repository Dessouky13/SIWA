...
export default function BlogChronicle() {
  const { t } = useLanguage();
  const accent = manifest?.socialAccent ?? manifest?.blog?.[0] ?? {};
    ...
    <section id="chronicle" className="bg-primary px-6 py-24 text-primary-foreground md:px-10">
      <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[0.9fr_1.1fr]">
        <div className="lg:sticky lg:top-16 lg:self-start">
          ...
          <div className="mt-8 overflow-hidden rounded-card border border-primary-foreground/15">
            <picture>
              <source srcSet={accent.webp} type="image/webp" />
              <img src={accent.jpg} alt="Traditional Siwan tea ceremony in the desert" loading="lazy" decoding="async" className="h-72 w-full object-cover heat-haze" />
            </picture>
          </div>
        </div>
        ...
        <div className="space-y-5">
          {postKeys.map((post) => (
            <article key={post.titleKey} className="rounded-card border border-primary-foreground/15 bg-primary-foreground/5 p-6 backdrop-blur transition hover:bg-primary-foreground/10">
              ...
              <blockquote className="mt-6 border-s-2 border-secondary ps-5 font-body text-lg italic leading-8 text-primary-foreground/80">"{t(post.quoteKey)}"</blockquote>
